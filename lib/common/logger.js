...
async function filterDeviceLogs(crawlerConfig) {
  ...
  if (crawlerConfig.platform.toLowerCase() === 'ios') {
    ...
    if (!crawlerConfig.deviceName && !crawlerConfig.udid) {
      logger.debug('invalid device config, both the udid and device name is not provided');
      return;
    }
    ...
    if (udid.length === 40 && !udid.includes('-')) {
      // device control
      cmd = `idevicesyslog -u ${udid} > ${path.join(process.cwd(), 'reports', root.crawlingDateStamp)}_ios.log &`;
    } else {
      // simulator control
      cmd = `xcrun simctl spawn ${udid} log stream > ${path.join(process.cwd(), 'reports', root.crawlingDateStamp)}_ios.log &`;
    }
    ...
    cmd = `adb -s ${crawlerConfig.deviceName || crawlerConfig.udid} logcat > ${path.join(process.cwd(), 'reports', root.crawlingDateStamp)}_android.log &`;

  } else {
    logger.debug('invalid platform information');
  }
