...
let date = new Date();
root.crawlingDateStamp = `${date.getFullYear()}-${date.getMonth()}-${date.getDate()}-${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}`;
...
module.exports = function(app) {
  ...
  rootRouter.get('/reports/:report/:resource', function *(next) {
    this.body = fs.readFileSync(path.join('.', this.originalUrl));
    this.type = 'image/png';
    yield next;
  });
