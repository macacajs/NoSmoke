...
NSCrawlerConfig.prototype.loadDefault = function() {
  ...
  if (global.cmdArgs && global.cmdArgs.config) {
    if (_.isExistedFile(global.cmdArgs.config)) {
      configFile = global.cmdArgs.config;
    } else {
      console.log('invalid config file paths :' + global.cmdArgs.config);
      process.exit();
    }
  }
  ...
  for (let i in global.cmdArgs) {
    if (this.hasOwnProperty(i) && global.cmdArgs[i]) {
      this[i] = global.cmdArgs[i];
    }
  }
