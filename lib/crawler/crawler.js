...
NSCrawler.prototype.refreshScreen = function (currentAction) {
  ...
  root.wdclient.send('/wd/hub/session/' + this.sessionId + '/screenshot', 'get', null, function(data) {
    ...
    const screenshootsDir = path.join(__dirname, '..', '..', 'reports', root.crawlingDateStamp);
    const fileName = `${_.uuid()}.png`;
    const fileDir = path.join('..', 'reports', root.crawlingDateStamp, fileName);

    _.mkdir(screenshootsDir);
    fs.writeFileSync(path.join(screenshootsDir, fileName), data.value, 'base64');
